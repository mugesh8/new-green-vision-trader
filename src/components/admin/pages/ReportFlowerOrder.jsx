...
const ReportFlowerOrder = () => {
    ...
    const fetchOrders = async () => {
      try {
        ...
        if (response.success && response.data) {
          // Filter to flower orders only, then sort by date (newest first)
          const flowerOrders = response.data.filter(o => o.order_type === 'flower' || o.order_type === 'FLOWER ORDER');
          const sortedOrders = flowerOrders.sort((a, b) => {
          ...
          // Fetch assignment data for each flower order (flower API only)
  ...
  const handleExportFilteredExcel = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, 'Flower Orders');
    XLSX.writeFile(wb, `Flower_Order_Report_${new Date().toISOString().split('T')[0]}.xlsx`);
  };
  ...
  const handleExportFilteredPDF = () => {
    const doc = new jsPDF();
    doc.setFontSize(16);
    doc.text('Flower Order Report', 105, 15, { align: 'center' });
    ...
    doc.save(`Flower_Order_Report_${new Date().toISOString().split('T')[0]}.pdf`);
  };
                    ...
                    <tr key={order.oid} className={`border-b border-[#D0E0DB] hover:bg-[#F0F4F3] transition-colors ${index % 2 === 0 ? 'bg-white' : 'bg-[#F0F4F3]/30'}`}>
                      ...
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-2">
                          <button
                            onClick={() => navigate(`/reports/flower-order/${order.oid}`)}
...
export default ReportFlowerOrder;
